(function(b) {
	...
	c.getElementOffset = function(a) {
		for(var c = a.offsetLeft, b = a.offsetTop; ( a = a.offsetParent) && a != document.body && a != document; )
	...
	c.createDOMDrawable = function(a, b) {
		var d = a.tagName || "div", g = b.image, i = a.width || g && g.width, j = a.height || g && g.height, k = c.createDOM(d);
		if(a.id)
			k.id = a.id;
		...
		k.style.height = j + "px";
		if(d == "canvas")
			k.width = i, k.height = j, g && ( d = k.getContext("2d"), i = b.rect || [0, 0, i, j], d.drawImage(g, i[0], i[1], i[2], i[3], a.x || 0, a.y || 0, a.width || i[2], a.height || i[3]));
		else if(k.style.opacity = a.alpha !=
			void 0 ? a.alpha : 1, k.style.overflow = "hidden", g && g.src)
			k.style.backgroundImage = "url(" + g.src + ")", k.style.backgroundPosition = -(a.rectX || 0) + "px " + -(a.rectY || 0) + "px";
		return k
	};
	c.DEG_TO_RAD = Math.PI / 180;
	c.RAD_TO_DEG = 180 / Math.PI;
	c.hitTestPoint = function(a, c, b, d) {
		var a = a.getBounds(), i = a.length, j = c >= a.x && c <= a.x + a.width && b >= a.y && b <= a.y + a.height;
		if(j && d) {
...
(function() {
	...
	b.prototype.concat = function(a) {
		var c = this.a, b = this.c, e = this.tx;
	...
	b.prototype.rotate = function(a) {
		var c = Math.cos(a), a = Math.sin(a), b = this.a, e = this.c, f = this.tx;
		this.a = b * c - this.b * a;
		this.b = b * a + this.b * c;
		this.c = e * c - this.d * a;
		this.d = e * a + this.d * c;
	...
	b.prototype.invert = function() {
		var a = this.a, c = this.b, b = this.c, e = this.d, f = this.tx, h = a * e - c * b;
		this.a = e / h;
		this.b = -c / h;
		this.c = -b / h;
		this.d = a / h;
	...
	b.prototype.transformPoint = function(a, c, b) {
		var e = a.x * this.a + a.y * this.c + this.tx, f = a.x * this.b + a.y * this.d + this.ty;
...
(function() {
	...
	b.prototype.intersection = function(a) {
		var c = Math.max(this.x, a.x), d = Math.min(this.x + this.width, a.x + a.width);
		if(c <= d) {
			var e = Math.max(this.y, a.y), a = Math.min(this.y + this.height, a.y + a.height);
			if(e <= a)
				return new b(c, e, d - c, a - e)
		}
		return null
	};
	b.prototype.union = function(a, c) {
		var d = Math.min(this.x, a.x), e = Math.min(this.y, a.y), f = Math.max(this.x + this.width, a.x + a.width) - d, h = Math.max(this.y + this.height, a.y + a.height) - e;
		if(c)
			return new b(d, e, f, h);
		else
			this.x = d, this.y = e, this.width = f, this.height = h
	};
...
(function() {
	...
	b.prototype.register = function(a, c, b, e, f) {
		...
		for(var h = {
			...
			for(var i = c[f], j = this._evtHandlers[i] || (this._evtHandlers[i] = []), k = 0, p = !1; k < j.length; k++) {
				var q = j[k];
	...
	b.prototype._onEvent = function(a, c, d) {
		...
		if(a.type.indexOf("touch") == 0)
			e = a.touches && a.touches.length > 0 ? a.touches[0] : a.changedTouches && a.changedTouches.length > 0 ? a.changedTouches[0] : a, e.type = f;
...
(function() {
	...
	b.prototype.removeEventListener = function(a, c) {
		...
		for(var e = 0; e < b.length; e++)
			if(b[e] === c)
				return b.splice(e, 1), b.length == 0 &&
				delete this._eventMap[a], !0;
		return !1
	};
	b.prototype.removeEventListenerByType = function(a) {
		return this._eventMap[a] != null ? (
		delete this._eventMap[a], !0) : !1
	};
...
(function() {
	...
	Quark.getUrlParams = function() {
		var e;
		var a = {}, c = window.location.href, b = c.indexOf("?");
		if(b > 0)
			for(var c = c.substring(b + 1).split("&"), b = 0, d; d = c[b]; b++)
				e = c[b] = d.split("="), d = e, a[d[0]] = d.length > 1 ? d[1] : !0;
		return a
	};
	var a = document.head, c = a.getElementsByTagName("meta"), d = c.length > 0 ? c[c.length - 1].nextSibling : a.childNodes[0];
	Quark.addMeta = function(c) {
		var b = document.createElement("meta"), h;
		for(h in c)
		b.setAttribute(h, c[h]);
		a.insertBefore(b, d)
	};
	Quark.toggleDebugRect = function(a) {
		...
		a._render = a.debug ? function(c) {
			...
			c = a.context.context;
			if(c != null)
				c.save(), c.lineWidth = 1, c.strokeStyle = "#f00", c.globalAlpha = 0.5;
			b(a, c);
			c != null && c.restore()
		} : function(c) {
			c.clear != null && c.clear(0, 0, a.width, a.height);
	...
	b.prototype._run = function() {
		if(!this.paused) {
			...
			for(var b = 0, e = this._listeners.length, f, h; b < e; b++)
				f = this._listeners[b], h = f.__runTime || 0, h == 0 ? f.step(this.info) : c > h && (f.step(this.info), this._listeners.splice(b, 1), b--, e--);
			a.lastTime = c
		}
	};
...
(function() {
	...
	b.prototype.getLoadedSize = function() {
		var a = 0, c;
		for(c in this._images)
		a += this._images[c].size || 0;
		return a
	};
...
(function() {
	var b = Quark.Tween = function(c, b, e) {
		...
		for(var f in b) {
			var h = c[f], g = b[f];
			h !==
			void 0 && typeof h == "number" && typeof g == "number" && (this._oldProps[f] = h, this._newProps[f] = g, this._deltaProps[f] = g - h)
		}
		for(f in e)
		this[f] = e[f]
	};
	...
	b.prototype._update = function() {
		if(!this.paused) {
			var a = Date.now(), d = a - this._startTime;
			if(!(d < 0)) {
				...
				this.target[e] = this._oldProps[e] + this._deltaProps[e] * this._reverseFlag * d;
				if(this.onUpdate != null)
					this.onUpdate(this, d);
				if(a >= 1) {
					if(this.reverse) {
						if( e = this._oldProps, this._oldProps = this._newProps, this._newProps = e, this._startTime = Date.now(), this._frameCount = 0, this._reverseFlag *= -1, !this.loop)
							this.reverse = !1
					} else if(this.loop) {
						for(e in this._oldProps)
						...
						this._frameCount = 0
					} else if(b.remove(this), e = this.next, e != null && ( e instanceof b ? ( a = e, e = null) : a = e.shift(), a != null))
						a.next = e, a.start();
	...
	a.Elastic.EaseInOut = function(a) {
		...
		if(a == 0)
			...
			f || ( f = 0.3);
		!e || e < 1 ? ( e = 1, b = f / 4) : b = f / (2 * Math.PI) * Math.asin(1 / e);
		return (a *= 2) < 1 ? -0.5 * e * Math.pow(2, 10 * (a -= 1)) * Math.sin((a - b) * 2 * Math.PI / f) : e * Math.pow(2, -10 * (a -= 1)) * Math.sin((a - b) * 2 * Math.PI / f) * 0.5 + 1
	};
...
(function() {
	...
	b.prototype._evtHandler = function(a) {
		if(a.type == "progress") {
			var b = 0, d = 0, e = a.target.buffered;
			if(e && e.length > 0)
				for( b = e.length; b >= 0; b--)
					d = e.end(b) - e.start(b);
			if(d / a.target.duration >= 1)
				this._element.removeEventListener("progress", this._evtHandler), this._element.removeEventListener("error", this._evtHandler), this._loaded = !0, this.dispatchEvent({
					type : "loaded",
					target : this
				}), this.autoPlay && this.play()
		} else
			a.type == "ended" ? (this.dispatchEvent({
				type : "ended",
				target : this
	...
	b.prototype.stop = function() {
		if(this._playing)
			this._element.pause(), this._playing = !1
	};
...
(function() {
	...
	b.prototype.get = function(a, b) {
		if(b == null || b.canvas.getContext != null)
	...
	b.prototype.set = function(a, b) {
		if( a instanceof HTMLImageElement || a instanceof HTMLCanvasElement || a instanceof HTMLVideoElement)
			this.rawDrawable = a;
		if(b === !0)
			this.domDrawable = a;
...
(function() {
	...
	b.prototype.getConcatenatedMatrix = function(a) {
		...
		for(var d = this; d.parent != null && d.parent != a; d = d.parent) {
			...
			b.concat(new Quark.Matrix(e * d.scaleX, f * d.scaleX, -f * d.scaleY, e * d.scaleY, d.x, d.y))
		}
		return b
	};
	b.prototype.getBounds = function() {
		var a = this.width, b = this.height, d = this.getConcatenatedMatrix(), a = this.polyArea || [{
			x : 0,
			y : 0
		}, {
			x : a,
...
(function() {
	...
	b.prototype.addChildAt = function(a, b) {
		if(b < 0)
			b = 0;
		else if(b > this.children.length)
		...
		a.parent = this;
		if(this.autoSize) {
			var d = new Quark.Rectangle(0, 0, this.rectWidth || this.width, this.rectHeight || this.height), e = new Quark.Rectangle(a.x, a.y, a.rectWidth || a.width, a.rectHeight || a.height);
			d.union(e);
			this.width = d.width;
	...
	b.prototype.removeChildAt = function(a) {
		...
		if(b != null)
			this.getStage().context.remove(b), b.parent = null;
	...
	b.prototype.swapChildren = function(a, b) {
		var d = this.getChildIndex(a), e = this.getChildIndex(b);
		this.children[d] = b;
		this.children[e] = a
	};
	b.prototype.swapChildrenAt = function(a, b) {
		var d = this.getChildAt(a), e = this.getChildAt(b);
		this.children[a] = e;
		this.children[b] = d
	};
	...
	b.prototype.getObjectUnderPoint = function(a, b, d, e) {
		...
		for(var h = this.children.length - 1; h >= 0; h--) {
			...
			if(!(g == null || !g.eventEnabled && g.children ==
				void 0 || !g.visible || g.alpha <= 0))
				if(g.children !=
				void 0 && g.eventChildren && g.getNumChildren() > 0) {
...
(function() {
	...
	b.prototype._onEvent = function(a) {
		var b = a.pageX - this.stageX, d = a.pageY - this.stageY, e = this._eventTarget, f = this.getObjectUnderPoint(b, d, !0);
		a.eventX = b;
		a.eventY = d;
		if(e != null && e != f)
			a.lastEventTarget = e, ( b = a.type == "mousemove" ? "mouseout" : a.type == "touchmove" ? "touchout" : null) && e._onEvent({
				type : b
			}), this._eventTarget = null;
		if(f != null && f.eventEnabled)
			a.eventTarget = e = this._eventTarget = f, f._onEvent(a);
		if(!Quark.supportTouch)
			this.context.canvas.style.cursor = e && e.useHandCursor && e.eventEnabled ? "pointer" : "";
		if(this.onEvent != null)
			this.onEvent(a)
	};
...
(function() {
	...
	b.prototype.setFrame = function(a, b) {
		b ==
		void 0 || b > this._frames.length ? b = this._frames.length : b < 0 && ( b = 0);
		this._frames[b] = a;
	...
	b.prototype.nextFrame = function(a) {
		...
		if(b.interval > 0)
			a = this._displayedCount + a, this._displayedCount = b.interval > a ? a : 0;
	...
	b.prototype.changeState = function(a) {
		if(this.state != a) {
			...
			switch(a) {
				...
				case b.UP:
					if(!this.enabled)
						this.eventEnabled = this.enabled = !0;
...
(function() {
	...
	var a = document.createElement("div"), c = a.style[Quark.cssPrefix + "Transform"] !=
	void 0, d = a.style[Quark.cssPrefix + "Perspective"] !=
	void 0, e = document.documentElement;
	...
	if(!c)
		throw "Error: DOMContext requires css transfrom support.";
	...
	h.prototype.draw = function(a) {
		if(!a._addedToDOM) {
			var b = a.parent, c = a.getDrawable(this);
	...
	h.prototype.transform = function(a) {
		...
		if(a.transformEnabled || !a._addedToDOM) {
			var d = Quark.cssPrefix, e = d + "TransformOrigin";
			d += "Transform";
			...
			if(!c.style[d] || a.propChanged("x", "y", "regX", "regY", "scaleX", "scaleY", "rotation"))
	...
	h.prototype.remove = function(a) {
		var b = a.getDrawable(this), c = b.parentNode;
