...
if (navigator.getUserMedia || navigator.webkitGetUserMedia) {
			...
			for ( var x = parseInt(xp1 - areaWid); x < parseInt(xp1 + areaWid); x++) {
				for ( var y = parseInt(yp1 - areaWid / 2); y < parseInt(yp1
					...
					if (h >= hhmin && h <= hhmax && s >= ssmin && s <= ssmax
						...
						// imgdata.data[redValueForPixel]=0;
						// imgdata.data[greenValueForPixel]=0;
						// imgdata.data[blueValueForPixel]=255;
					}
				}
			}
			...
			for ( var x = parseInt(xp2 - areaWid / 2); x < parseInt(xp2
				...
				for ( var y = parseInt(yp2 - areaWid); y < parseInt(yp2
					...
					if (h >= hhmin && h <= hhmax && s >= ssmin && s <= ssmax
						...
						// imgdata.data[redValueForPixel]=255;
						// imgdata.data[greenValueForPixel]=0;
						// imgdata.data[blueValueForPixel]=0;
					}
				}
			}
	...
	worker.onmessage = function(event) {
		...
		if (state1 == 2 && state2 == 2) {
			...
			setCookie("forefingerHmin", hmin);
			setCookie("forefingerSmin", smin);
			setCookie("forefingerVmin", vmin);
			setCookie("forefingerHmax", hmax);
			setCookie("forefingerSmax", smax);
			setCookie("forefingerVmax", vmax);

			window.location = "checkThumbView.html";
		} else {
			recognize();
		}
	};
