...
(function() {
    Canvas.Img = function(el, oConfig) {
        /// this.rotateImage = new YY.util.CustomEvent('rotateImage', this);
    ...
    Canvas.Img.prototype.position = null;
    ...
    Canvas.Img.prototype._initElement = function(el) {
        if (YY.util.Dom.inDocument(el)) {
            ...
            YY.util.Dom.addClass(this._oElement, Canvas.Img.CSS_CANVAS);
        }
    };
    Canvas.Img.prototype._initConfig = function(oConfig) {
        ...
        for (sKey in DEFAULT_CONFIG) {
            ...
            if (!oConfig.hasOwnProperty(defaultKey)) {
        ...
        this.width = normalizedSize.width;
        this.height = normalizedSize.height;
        if (this.position) {
            this._setProperties(oConfig);
        }
    ...
    Canvas.Img.prototype.getOriginalSize = function() {
        return {
            width: this._oElement.width,
            height: this._oElement.height
        }
    };
    Canvas.Img.prototype._setProperties = function(oConfig) {
        if (oConfig.angle == null) {
            this.angle = (Math.random() * 90) - 45;
        }
        if (oConfig.top == null) {
            this.top = parseInt(Math.random()*(270)+180);
        }
        if (oConfig.left == null) {
            this.left = parseInt(Math.random()*(300)+500);
        }
    };
